...
export const onRequestGet = async ({ env }) => {
  try {
    const [gifts, claims] = await Promise.all([
      supabaseGet(env, 'gifts', '?select=id,name,category,description,price,image_url,badge,featured,payment_url,pix_key,pix_payload,available&available=eq.true&order=featured.asc'),
