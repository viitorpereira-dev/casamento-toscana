...
const getGift = async (env, giftId) => {
  const rows = await supabaseGet(env, 'gifts', `?select=id,name,price,payment_url,pix_key,pix_payload,available&id=eq.${encodeURIComponent(giftId)}&limit=1`);
  return rows[0] || null;
};
...
const createClaim = async (env, body) => {
  ...
  const paymentMethod = body.paymentMethod === 'pix' || body.paymentMethod === 'mercado-pago' ? body.paymentMethod : '';

  if (!giftId || guestName.length < 2 || !isValidEmail(guestEmail) || !paymentMethod) {
    return errorResponse('Informe nome, e-mail e forma de pagamento.', 422);
  }
